    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
     ...
     <div class="contaniner-fluid">
      ...
      <h1 style="text-align:center; color:white;">Minions Stats:</h1>
        ...
        function Damage(int &$DMG, int &$HP, int $STR, int $DEF) {
          $DMG = $STR - $DEF;
          $HP = $HP - $DMG;
        }

        function Tim_Attacks(int $DMG, int &$HP, int $STR, int $DEF) {
          Damage($DMG, $HP, $STR, $DEF);
          echo "<rev style=\"color:white; margin-left: 10px; font-size: 30px; \">Tim attacks for: $DMG .</rev>";
          echo "<br>";
          echo "<rev style=\"color:white; margin-left: 10px; font-size: 30px; \">Enemy health becomes: $HP.</rev>";
          echo "<br>";
        }

        function Enemy_Attacks(int $DMG, int &$HP, int $STR, int $DEF) {
          Damage($DMG, $HP, $STR, $DEF);
          echo "<rev style=\"color:white; margin-left: 10px; font-size: 30px; \">Enemy attacks for: $DMG .</rev>";
          echo "<br>";
          echo "<rev style=\"color:white; margin-left: 10px; font-size: 30px; \">Tim health becomes: $HP.</rev>";
          echo "<br>";
        }
        ...
        $i = 2;
        echo "<h2 style=\" color:white; margin-left: 15px;\">Atack 1:</h2>";
        if($Tim->Speed > $Enemy->Speed) 
        {
          echo "<p style=\"color:white; margin-left: 10px; font-size: 30px; \">Tim atacks first!</p>";
          echo Tim_Attacks($Tim->Dmg, $Enemy->Health, $Tim->Strength, $Enemy->Defense);

          while($i < 20) {
            if($Tim->Health < 0 || $Enemy->Health < 0) {
              if($Tim->Health > $Enemy->Health) 
              echo "<h1 style=\"text-align:center; color:white; margin-left: 10px;\">Tim WON!</h1>";
              else 
              echo "<h1 style=\"text-align:center; color:white; margin-left: 10px;\">Enemy WON!</h1>";
              break;
            }
            else
            {
              echo "<h2 style=\" color:white; margin-left: 15px;\">Atack $i:</h2>";
              echo Enemy_Attacks($Enemy->Dmg, $Tim->Health, $Enemy->Strength, $Tim->Defense);
              if($Tim->Health < 0 || $Enemy->Health < 0)
              {
                if($Tim->Health > $Enemy->Health)
                echo "<h1 style=\"text-align:center; color:white; margin-left: 10px;\">Tim WON!</h1>";
        ...
        echo "<p style=\"color:white; margin-left: 10px; font-size: 30px; \">Enemy atacks first!</p>";
        echo Enemy_Attacks($Enemy->Dmg, $Tim->Health, $Enemy->Strength, $Tim->Defense);
